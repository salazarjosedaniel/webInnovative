...
static void httpWorkerTask(void* pv) {
  ...
  for (;;) {
    ...
    char ivaText[64];
    if (taxPercent > 0.0) {
      snprintf(ivaText, sizeof(ivaText), "IVA (%.0f%%): %s MXN.", taxPercent, bufIva);
    } else {
      snprintf(ivaText, sizeof(ivaText), "IVA: %s MXN.", bufIva);
    }
...
void setup() {
  ...
  if (usarLVGL) {
    ...
    if (!okPanel) {
      ...
      lv_label_set_text(lblIva, "IVA: --.-- MXN.");
