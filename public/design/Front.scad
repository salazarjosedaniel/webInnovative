...
// Si tu STL te quedó "invertido" (lector arriba / orejas abajo),
// pon esto en true y exporta de nuevo.
...
window_w = 156.88;  // ventana visible (LCD)
window_h = 88.72;
window_x = 19.04;
window_y = 13.04;
...
module front_bezel(){
  ...
  module bezel_body(){
    difference(){
      ...
      // Si flip body Y, el texto queda espejado verticalmente -> lo corregimos
    ...
    // Bosses con insertos M3 por detrás (no visibles al frente)
    for (sx=[-1,1], sy=[-1,1]){
      insert_boss(ux(sx), uy(sy), bezel_th, bezel_insert_od, bezel_insert_h, m3_inset_d);
    }
  }

  // Render: flip solo el cuerpo si se requiere (en bezel no hay orejas)
...
module back_case(){
  ...
  // Cuerpo de la caja (sin orejas)
  module back_body(){
    difference(){
      // UNA SOLA PIEZA (contorno continuo)
      linear_extrude(height=case_depth)
        outline2d_continuo(ow, oh, bottom_extension_h, corner_r, ext_corner_r);

      // Cavidad interior continua
  ...
  // Render:
  // 1) Cuerpo (opcionalmente invertido)
  // 2) Orejas siempre arriba (si KEEP_EARS_ALWAYS_TOP)
  union(){
    maybe_flip_body_y() back_body();

    if (KEEP_EARS_ALWAYS_TOP){
      // Orejas en coordenadas "normales" (arriba real)
      ears_top_only();
    } else {
      // Orejas siguen al cuerpo (si inviertes, también bajan)
